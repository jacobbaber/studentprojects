...
The result is that the Automaton constructors initialize the rule field without knowing the subclass of the Rule object they create.<sup id="a3">[3](#f3)</sup>
...
Dependency injection can't be used here because Automaton has a constructor that creates a Rule object with a number it reads from a text file.
To use dependency injection, the Rule must be created before calling the Automaton constructor, and this is impossible if the rule number is unknown.
